        ...
        <div class="px-4 xl:pl-16">
            <div class="mb-4 md:flex md:justify-between xl:pr-16">
                ...
                <div class="flex space-x-4 mb-4 mt-5 md:mt-0">
                    <button class="hover:text-[#a5d8f1] border border-primary relative px-2 py-2 rounded-full " v-for="category in ['All', 'Web Development', 'Mobile App']"
                        :key="category" @click="() => selectedCategory = category"
                        @touchstart="selectedCategory = category" >
                        {{ category }}
                    </button>
                </div>
            </div>
            <ul class="px-4 sm:py-16 xl:pr-16 grid grid-cols-1 gap-6 pt-10 sm:grid-cols-2 md:gap-10 md:pt-12 lg:grid-cols-3"
                ...
                <div v-for="project in filteredProjects" :key="project.id">
                    <div class="h-52 md:h-[24rem] rounded-t-xl relative group border-[1px] border-[#a5d8f1]"
